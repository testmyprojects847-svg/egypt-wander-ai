  ...
  return (
    <section className="bg-black py-10 md:py-14 px-6 md:px-16">
      <div className="max-w-7xl mx-auto">
        <div className={`flex flex-col md:flex-row items-center justify-between gap-6 md:gap-4 ${isRTL ? 'md:flex-row-reverse' : ''}`}>
          {/* Pharaoh Image */}
          <motion.div
            initial={{ opacity: 0, x: isRTL ? 50 : -50 }}
            animate={{ opacity: 1, x: 0 }}
            transition={{ duration: 0.8 }}
            className={`flex-1 flex justify-center ${isRTL ? 'md:justify-end' : 'md:justify-start'}`}
          >
          ...
          >
            <h1 className="font-playfair text-primary tracking-[0.25em] leading-[1.15]">
              <span className="block text-3xl md:text-4xl lg:text-5xl xl:text-6xl">{t('explore')}</span>
              <span className="block text-3xl md:text-4xl lg:text-5xl xl:text-6xl">{t('modern')}</span>
              <span className="block text-3xl md:text-4xl lg:text-5xl xl:text-6xl">{t('egypt')}</span>
            </h1>
            <div className="mt-6">
